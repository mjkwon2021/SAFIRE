...
Evaluate Two-source performance.
Compatible: networks/safire_predictor_binary.py, networks/safire_model.py
"""
...
def show_mask_one(anns, ax, index, max_confidence_indices=None, cluster_0_indices=None, random_color=False, color=None):
    if len(anns) == 0:
        return
    ...
    if index in max_confidence_indices and index not in cluster_0_indices:
        show_points(torch.FloatTensor(coord[0]).unsqueeze(0), torch.LongTensor((1,)), ax, marker='D', color='green',
                    marker_size=80)
    elif index in max_confidence_indices and index in cluster_0_indices:
        show_points(torch.FloatTensor(coord[0]).unsqueeze(0), torch.LongTensor((1,)), ax, marker='D', color='blue',
                    marker_size=80)
    elif index not in cluster_0_indices:
        show_points(torch.FloatTensor(coord[0]).unsqueeze(0), torch.LongTensor((1,)), ax, marker='.',  color='green', marker_size=200)
    else:
        show_points(torch.FloatTensor(coord[0]).unsqueeze(0), torch.LongTensor((1,)), ax, marker='.', color='blue', marker_size=200)
...
def main():
    ...
    safire_automatic_model = SafirePredictor(medsam_model, points_per_side=8, points_per_batch=64*4, pred_iou_thresh=0, stability_score_thresh=0.0, box_nms_thresh=0.0)
    ...
    for dataset_name, test_forensic_dataset in test_forensic_datasets.items():
        ...
        with torch.no_grad():
            for step, (image, gt2D, img_paths) in enumerate(
                ...
                anns, safire_pred, max_confidence_indices, cluster_0_indices = safire_automatic_model.safire_predict(npimage)
                ...
                if args.imsave and step % 1 == 0:
                    ...
                    plt.savefig(fname=str(save_dir / (str(Path(img_paths[0]).name)+"_im_gt_pred.png")), bbox_inches='tight', pad_inches=0)
                    plt.close()
                ...
                if args.imsave and step % 1 == 0:
                    ...
                    for i in range(min(len(anns), 64)):
                        plt.subplot(8,8,1+i)
                        plt.axis('off')
                        plt.title(f"{anns[i]['predicted_iou']:.04f}", fontsize=12)
                        plt.imshow(np.zeros_like(image[0].cpu()))
                        show_mask_one(anns, plt.gca(), i, max_confidence_indices, cluster_0_indices)
                    save_dir = save_path / f"pred_{run_id}" / dataset_name
                    os.makedirs(save_dir, exist_ok=True)
                    print(img_paths[0])
                    plt.savefig(fname=str(save_dir / (str(Path(img_paths[0]).name)+"_masks.png")), bbox_inches='tight', pad_inches=0)
                    plt.close()
