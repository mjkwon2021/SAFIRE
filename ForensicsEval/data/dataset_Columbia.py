...
class Dataset_Columbia(AbstractForgeryDataset):
    """
    directory structure
    Columbia Uncompressed Image Splicing Detection (dataset_path["Columbia"] in project_config.py)
    ├── 4cam_splc (uncompress it)
    │   ├── edgemask
    │   └── canong3_canonxt_sub_01.tif ... (total 180 images)
    └── 4cam_auth (uncompress it)
        └── canong3_02_sub_01.tif ... (total 183 images)
    """
    def __init__(self, im_list_file):
        ...
        """
        super().__init__()
        self.root_path = project_config.dataset_paths['Columbia']
